...
| `install.py` (`_prompt_claude_statusline`) | "Install the Claude Code statusline?" (only when Claude Code is detected) | Yes | `install.claude.statusline` | yes/no | **Yes** — fixed payload (AEC's own `.claude/statusline.sh`); never overwrites an existing `statusLine` | `settings.claude_statusline` + `~/.claude/settings.json` `statusLine` |
