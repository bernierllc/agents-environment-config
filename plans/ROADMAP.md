...
| Managed symlink ownership by record | `is_our_symlink` recognises AEC links by a path-name substring, so checkouts cloned under another name leave dangling, never-repaired links after a move or reclone. Record link ownership at creation instead. | `docs/superpowers/plans/2026-09-25-managed-symlink-ownership.md` *(proposed placement — needs confirmation)* | High — root cause + fix scoped |
