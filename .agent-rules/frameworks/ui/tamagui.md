...
Add `testID` to all interactive elements, dynamic state containers, and structural landmarks when building components. React Native uses `testID` (not `data-testid`).
...
{hasError && (
  <Text testID="error-message" col="$red10">
    {errorMessage}
  </Text>
)}

<YStack testID="main-screen" flex={1}>
  {/* screen content */}
</YStack>
```

Static/decorative elements (labels, icons, descriptive text) do not need `testID`. Missing `testID` on required elements is a developer deficiency that must be fixed at the source — do not work around it in tests.
