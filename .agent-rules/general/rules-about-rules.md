...
## Core Principle
Rules are living documents and should be updated as we learn. Suggest improvements when something is unclear or not working. Keep rules concise and merge overlapping rules where possible.

## Global vs Project-Specific Rules

### Global Rules (This Repository)

These rules in `agents-environment-config/.cursor/rules/` are **global rules** that apply across all projects. They contain:
...
Project-specific rules stay in their respective projects (`{project}/.cursor/rules/`) and should:
...
For detailed architecture, see `plans/cursor/architecture.md`.
